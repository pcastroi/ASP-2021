...
if nargout == 0
    ...
    plot(ind1/fs,L1a,'ko',ind2/fs,L2a,'kx');
    line([rt60 rt60],[0 -100],'LineStyle','--');
    ylim([-100 0]);
    xlabel('T [s]'); ylabel('EDC [dB]');
    subtitle(sprintf('RT_{60} = %.2f s',rt60));
    legend('Impulse Response','EDC','lin.fit',strcat('$T_{', num2str(round(L1a)), '}$'),strcat('$T_{', num2str(round(L2a)), '}$'),'Reverb Time','Interpreter','latex');
end
end
