...
general.fs = 44100; % Sampling frequency, in Hertz
...
dsp.devicePlayName = 'ASIO Fireface USB'; % Playing Device ID (run HeaAudioDSP.listDevices to get the list)
dsp.deviceRecName = 'ASIO Fireface USB'; % Recording Device ID (run HeaAudioDSP.listDevices to get the list)
dsp.bufferSize = 1024; % Buffer size, in samples (type "doc dsp.audioplayer" and check the first figure)
dsp.queueDuration = 0.1; % Queue duration, in seconds (type "doc dsp.audioplayer" and check the first figure)
dsp.samplesPerFrame = 1024; % Samples per frame, in samples (type "doc dsp.audioplayer" and check the first figure)
